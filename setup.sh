...
I18N_EN["cannot_detect_os"]="Cannot detect OS: /etc/os-release not found"
I18N_EN["starting"]="Starting"
I18N_EN["sshd_not_found"]="SSH server (sshd) not found. Install openssh-server first"
I18N_EN["enable_ufw_prompt"]="Enable UFW firewall now?"
I18N_EN["ufw_reset_failed"]="Failed to reset UFW rules"
I18N_EN["exit_code"]="exit code"
I18N_EN["for_ssh"]="for SSH"
I18N_EN["fail2ban_not_running"]="Fail2Ban service is not running. Check: systemctl status fail2ban"
I18N_EN["all_packages_installed"]="All packages already installed"
I18N_EN["failed_set_password"]="Failed to set password"
I18N_EN["invalid_ssh_key"]="Invalid SSH public key. Expected format: ssh-ed25519 AAAA... comment"
I18N_EN["invalid_domain"]="Invalid domain name (e.g., example.com)"
I18N_EN["3xui_info"]="3x-ui access information"
I18N_EN["3xui_see_output"]="Credentials were generated by the installer (see its output above). Run 'x-ui' to manage the panel"
I18N_EN["3xui_ufw_hint"]="If UFW is enabled, allow the panel port: ufw allow <port>/tcp"
...
# Ключи, добавленные в 1.2.0
I18N_RU["required"]="требуется"
I18N_RU["cannot_detect_os"]="Не удалось определить ОС: файл /etc/os-release не найден"
I18N_RU["starting"]="Запуск"
I18N_RU["sshd_not_found"]="SSH-сервер (sshd) не найден. Сначала установите openssh-server"
I18N_RU["enable_ufw_prompt"]="Включить фаервол UFW сейчас?"
I18N_RU["ufw_reset_failed"]="Не удалось сбросить правила UFW"
I18N_RU["exit_code"]="код возврата"
I18N_RU["for_ssh"]="для SSH"
I18N_RU["fail2ban_not_running"]="Служба Fail2Ban не запущена. Проверьте: systemctl status fail2ban"
I18N_RU["all_packages_installed"]="Все пакеты уже установлены"
I18N_RU["failed_set_password"]="Не удалось установить пароль"
I18N_RU["invalid_ssh_key"]="Неверный публичный SSH-ключ. Ожидаемый формат: ssh-ed25519 AAAA... комментарий"
I18N_RU["invalid_domain"]="Неверное имя домена (например, example.com)"
I18N_RU["3xui_info"]="Информация для доступа к 3x-ui"
I18N_RU["3xui_see_output"]="Учётные данные сгенерированы установщиком (см. его вывод выше). Для управления выполните 'x-ui'"
I18N_RU["3xui_ufw_hint"]="Если UFW включен, откройте порт панели: ufw allow <порт>/tcp"
...
check_os() {
    ...
    if [[ "$supported" != true ]]; then
        err "$(_ "unsupported_os"): $PRETTY_NAME ($(_ "required"): Ubuntu ${MIN_UBUNTU_VERSION}+ / Debian ${MIN_DEBIAN_VERSION}+)"
        return 1
    fi

    info "$(_ "os_detected"): $PRETTY_NAME ($VERSION_ID)"
    return 0
}
...
check_arch() {
    ...
    case "$arch" in
        x86_64|aarch64|arm64)
            info "$(_ "architecture"): $arch"
            return 0
            ;;
        *)
            err "$(_ "unsupported_arch"): $arch"
            return 1
            ;;
    esac
}
...
check_kernel_version() {
    ...
    if [[ "$(printf '%s\n' "$required" "$current" | sort -V | head -n1)" != "$required" ]]; then
        err "$(_ "kernel_too_old") $required ($current)"
        return 1
    fi
    return 0
}
...
wait_apt_lock() {
    ...
    while true; do
        ...
        if [[ $waited -ge $max_wait ]]; then
            err "$(_ "apt_lock_timeout") ${max_wait}s"
            return 1
        fi
        info "$(_ "waiting_apt") (${waited}s)"
        sleep 5
        waited=$((waited + 5))
    done
    return 0
...
apt_update() {
    ...
    if [[ $rc -ne 0 ]]; then
        err "$(_ "update_failed") ($(_ "exit_code"): $rc)"
        echo "$output" | tail -20 >&2
        return 1
    fi
    ok "$(_ "package_list_updated")"
    return 0
}
...
install_package() {
    ...
    if package_installed "$pkg"; then
        ok "$(_ "package_already_installed"): $pkg"
        return 0
    fi

    info "$(_ "installing"): $pkg"
    wait_apt_lock || return 1
    ...
    if [[ $rc -eq 0 ]]; then
        ...
        err "$(_ "failed_to_install"): $pkg ($(_ "exit_code"): $rc)"
...
backup_file() {
    ...
    if [[ -f "$file" ]]; then
        ...
        cp "$file" "$backup_name"
        ok "$(_ "backed_up"): $file -> $backup_name"
    fi
}
...
test_ssh_config() {
    if sshd -t 2>/dev/null; then
        ok "$(_ "ssh_test_passed")"
        return 0
    else
        err "$(_ "ssh_test_failed")"
        return 1
    fi
}
...
restart_service() {
    local service="$1"
    info "$(_ "restarting_service"): $service"
    ...
    if [[ $rc -eq 0 ]]; then
        ok "$(_ "service_restarted"): $service"
        return 0
    else
        err "$(_ "failed_to_restart"): $service ($(_ "exit_code"): $rc)"
...
enable_service() {
    ...
    if systemctl enable "$service" 2>/dev/null; then
        ok "$(_ "service_enabled"): $service"
        return 0
    else
        err "$(_ "failed_to_enable"): $service"
        return 1
    fi
}
...
configure_firewall() {
    ...
    ok "$(_ "allowed_port"): $ssh_port ($(_ "for_ssh"))"
    ...
    # Enable UFW
    if confirm "$(_ "enable_ufw_prompt")" "Y"; then
        ufw --force enable
        ok "$(_ "ufw_enabled")"
    else
...
speed_test() {
    ...
    if [[ $rc -eq 0 ]]; then
        ...
        err "$(_ "speedtest_failed") ($(_ "exit_code"): $rc)"
        echo "$output" >&2
        return 1
    fi
}
...
install_3xui() {
    ...
    if confirm "$(_ "3xui_confirm")" "Y"; then
        ...
        if [[ $rc -ne 0 ]]; then
            err "$(_ "3xui_install_failed") ($(_ "exit_code"): $rc)"
            return 1
        fi

        ok "$(_ "3xui_completed")"

        # The installer generates random credentials and, in unattended mode,
        # writes them to /etc/x-ui/install-result.env — show the real values
        # instead of assuming hardcoded defaults
        echo -e "\n${BOLD}$(_ "3xui_info"):${NC}"
        if [[ -f /etc/x-ui/install-result.env ]]; then
            sed 's/^/  /' /etc/x-ui/install-result.env
        else
            echo -e "  $(_ "3xui_see_output")"
        fi
        warn "$(_ "3xui_ufw_hint")"
...
main() {
    ...
    info "$(_ "starting") $SCRIPT_NAME v$SCRIPT_VERSION"
